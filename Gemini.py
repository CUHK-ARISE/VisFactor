...
def chat(payload,example,key):
    time.sleep(5)
    contents = [payload['messages'][0]['content']]
    for i in range(len(payload['messages'][1]['content'])):
        content = payload['messages'][1]['content'][i]
        if content['type'] == 'text':
            contents.append(content['text'])
        else:
            b64_image = types.Part.from_bytes(
                data=pathlib.Path(content['image_path']).read_bytes(),
                mime_type="image/jpeg"
            )
            contents.append(b64_image)
...
def do_one_test(test_meta, test_mode, use_example, use_cot, test_prefix, key):
    ...
    for i, (q, a) in enumerate(zip(test_meta[test_mode]["Questions"], test_meta[test_mode]["Answers"])):
        ...
        for _ in range(0, _MAX_TRY):
            ...
            full_json = chat(query_request,examples,key)
            # print(full_json)
            try:
